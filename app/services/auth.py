...
def user_sign_up(request_data: dict) -> jsonify:
    ...
    if already_existing_user:
        return create_response('already existing email', 'failed', 409)
    ...
    return create_response('Successfully registered.', 'success', 201, result)
...
def login(request_data: dict) -> jsonify:
    ...
    if not user:
        return create_response('LogIn failed', 'failed', 400)

    if not check_password_hash(user.password, password):
        return create_response('Log In failed', 'failed', 400)
    else:
        result = {
            'auth_token': create_auth_token(user)
        }
        return create_response('Log In successful', 'success', 200, result)
